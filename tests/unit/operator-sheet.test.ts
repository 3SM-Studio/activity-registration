...
describe("operator Sheets experience", () => {
  it("hides technical columns without hiding operator fields", () => {
  ...
  it("adds the three operator filter views when missing", () => {
    const requests = buildOperatorSheetRequests(sheet());
    const titles = requests.flatMap((request) => {
      const add = request.addFilterView as { filter?: { title?: string } } | undefined;
      return add?.filter?.title ? [add.filter.title] : [];
    });

    expect(titles).toEqual(Object.values(REGISTRATION_OPERATOR_FILTER_VIEW_TITLES));
  });

  it("updates an owned view and deletes duplicate copies", () => {
  ...
  it("replaces only owned conditional formats", () => {
